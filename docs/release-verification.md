...
For dormant P4.5 application-package prototypes, additionally run `npm run verify:presentation-prototypes`. Add `-- --keep` for visual review. It must produce eight synthetic matching packages (Northstar, Civic, Studio Editorial v2, and Essential across one short Canadian and one long U.S. fixture), 32 DOCX/PDF files, one-page short résumés, two-page long résumés, one-page letters, canonical manifest/letter parity, distinct presentation hashes, and invariant résumé content hashes. The detailed local evidence and remaining hands-on gates are recorded in `docs/p4-5-prototype-evidence.md`.

1. Run focused canonical/template/export/tailoring tests:
   `node --test src/ResumeExperience.test.js src/resumeModel.test.js src/resumeUniversalTemplates.test.js src/resumeParity.test.js src/resumePresentation.test.js src/resumeTemplateStorage.test.js src/resumeDocx.test.js src/resumePdf.test.js src/resumeStrategy.test.js tests/resumeQuality.test.js tests/tailoringEvidence.test.js tests/safeResumeFallback.test.js`
2. Run `npm run verify:exports`. This creates final DOCX/PDF fixtures for 16 strategy/style compatibility combinations covering all nine content strategies and all seven visual styles, plus four representative palette/density/header/length combinations, an apprentice Skilled Trades / Field Services pair, Marketing transition and Creative adjacent pairs, and a preliminary ATS Core pair. Technical / Software, Admin / Customer Operations, experienced Skilled Trades / Field Services, Marketing & Communications, and Creative & Design use realistic two-page fixtures; the apprentice and both adjacent B3 fixtures must remain one page. The verifier compares DOCX, selectable PDF, and plain text against the canonical manifest, checks package/PDF structure, reading order, heading-to-first-content page cohesion, and compact-project pagination, rejects object/metadata artifacts, and exercises fresh/stale authorization and identity gates before removing temporary files.
...
6. On desktop and at 390 × 844, open the seven-style gallery using keyboard and pointer input. Verify focus visibility, `aria-expanded`/`aria-pressed` state, touch targets, immediate rerender without a network request, account/target persistence, safety labels, unchanged `data-resume-strategy`/`data-resume-content-hash`, and the expected style/palette/density/header/length data-attribute updates.

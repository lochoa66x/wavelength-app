...
| Candidate Evidence Coach | One reviewed requirement/question and only the factual answer fields entered for that requirement; never the full résumé | Proposal stays in the target-scoped browser evidence draft until the candidate approves, edits, or rejects it | Authenticated `/api/evidence-coach`; no database write | None | One bounded clarification call; missing facts return one follow-up instead of guessed evidence | Coarse outcome/disposition/confidence/count/duration metadata only; no answer, requirement, citation, or proposal text; response is `no-store` |

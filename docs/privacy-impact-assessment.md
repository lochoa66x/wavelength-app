...
| Cover letter silently overstates motivation or fit, or copies private gap analysis into employer-facing prose | High | Every substantive paragraph carries exact résumé/posting citations; the generation prompt excludes the private fit assessment; generic flattery, unsupported motivation, placeholders, unsupported numbers, transition framing, gap confessions, and conditional-candidacy language are rejected; legacy self-disqualifying drafts and paragraph edits are blocked from export until replaced or rechecked | Candidate still owns final review and submission |

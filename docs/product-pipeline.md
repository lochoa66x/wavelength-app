...
- Generate a cover letter only from the reviewed posting, canonical résumé facts, and confirmed candidate evidence. Keep the application-risk assessment candidate-side: it may guide the private review experience, but it is excluded from the generation prompt and employer-facing letter so gap analysis cannot become a self-disqualifying disclosure. A missing requirement remains missing and cannot become motivation, experience, or a claimed strength; it is simply omitted from the letter.
- Treat the letter as truthful advocacy: lead with the strongest verified experience, skills, results, leadership, and adjacent domain foundations; never volunteer missing qualifications, gap inventories, career-transition framing, conditional candidacy, or reasons to reject the applicant. Limit the full letter to three or four focused paragraphs.
...
- **P4.6A application-document workspace — released and production-verified on 2026-09-02:** application intake now begins with an explicit choice among a matching résumé-and-letter package, résumé only, and cover letter only. Cover-letter-only runs the shared posting/evidence analysis but does not request or create a hidden tailored résumé. Existing résumé results show a compact target-level document summary and an early matching-letter action before the long preview; package mode opens the letter workspace automatically. Résumé and letter readiness remain independent, the base résumé stays immutable, legacy target-scoped cover-letter drafts are discovered without rewriting them, and new target/status metadata remains account-and-target-scoped in browser storage with no Supabase/schema/sync expansion. Scoped deletion and the public privacy record include the new metadata. Generation, exports, cancellation, stale checks, and sign-in/private-processing gates continue to use the existing evidence-first contracts. Feature commit `a94036d9ebbe85ac31b4f170f75d7b4bb748d215` is on `origin/main` and is `READY` at <https://gigscapes.com> through Vercel deployment `dpl_4LdvPAMCLtKAiMMkkUrrtYFbFQhm`; automated, build, binding, alias, public-route, private-boundary, and runtime-log evidence is recorded in `docs/release-checklist.md`. Authenticated desktop/mobile and external office-suite/ATS hands-on testing remain.
- **P4.6B application tracker — later:** consider a lightweight saved → applied → interviewing → offer tracker only after P4.6A tester evidence is stable. It requires a deliberate persistence, retention, deletion, conflict, multi-device, and analytics decision. Do not add automatic application submission.

...
import { comparisonQueue } from '../evaluations/comparative-v1/cases.js';

export default function ComparisonExperiment() {
  const { session, loading } = useAuth();
  const [capability, setCapability] = useState('');
  const [results, setResults] = useState([]);
  const [status, setStatus] = useState('Ready to run 48 frozen first requests.');
  ...
  async function run() {
    ...
    for (const cell of comparisonQueue) {
      ...
      setStatus(`${captured.length}/48 complete. Running ${cell.caseId} ${cell.arm} ${cell.kind}.`);
      try {
        ...
        // Access/transport errors stop the queue; do not burn 48 failures on a broken runner.
        stop.current = true;
      }
      setResults([...captured]);
    }
    setRunning(false);
    setStatus(`${captured.length}/48 complete.${stop.current ? ' Stopped; captured attempts are preserved.' : ' Experiment complete.'}`);
  }
  function download() {
  ...
  return <main style={{ maxWidth: 960, margin: '40px auto', padding: 24, fontFamily: 'system-ui' }}>
    ...
    <p>Six fictional candidates, four controlled approaches. Each request and failure is retained. Your saved résumé is never read or changed here.</p>

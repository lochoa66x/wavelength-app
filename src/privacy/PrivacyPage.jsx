      ...
      <article className="privacy-document">
        ...
        <PolicySection title="2. What Gigscapes handles">
          <ul>
            ...
            <li><strong>Application-document workspace:</strong> résumé text, confirmed candidate evidence, presentation choices, target-specific cover-letter drafts, and small application-package status records saved in this browser for the signed-in account. A status record contains the posting title, company, location, selected document types, document states, and source hashes—not the full posting or résumé. DOCX and text-based PDF imports are parsed on the device; their original files are not saved or synced. {RESUME_SYNC_ENABLED ? "If you explicitly enable résumé sync, an account-owned copy of the reviewed base-résumé text is also stored in Supabase." : null}</li>
        ...
        <PolicySection title="3. Where data is stored">
          <p>Your base résumé, target-specific cover-letter drafts, and application-package status records are saved in local browser storage on the device where you entered or generated them. Browser-only storage is the default. Clearing browser data, using a different browser profile, or choosing the local deletion control can remove that device copy.</p>
        ...
        <PolicySection title="7. Retention and deletion">
          <p>Local résumé, cover-letter, and application-package status data remains until the browser or user removes it. The local deletion control removes the current account’s résumé, target-specific cover-letter drafts, application-package status records, confirmed evidence, presentation choices, sync preference, and processing acknowledgement from that browser only. It does not delete an account-synced résumé, the Supabase account, saved jobs, search preferences, authentication session, or copies a provider must temporarily retain for security or legal obligations.</p>

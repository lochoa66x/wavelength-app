...
export async function createCoverLetterDocxBlob(input) {
  ...
  paragraph(plan.candidate.fullName, { alignment: headerAlignment, spacing: { after: 45 }, ...(headerRule.top ? { border: { top: headerRule.top, ...(!plan.candidate.contactLine ? { bottom: headerRule.bottom } : {}) } } : !plan.candidate.contactLine ? { border: headerRule } : {}), run: { bold: true, font: tokens.docxDisplayFontFamily, size: Math.round(tokens.nameFontSizePt * 2) } });
  if (plan.candidate.contactLine) paragraph(plan.candidate.contactLine, { alignment: headerAlignment, border: { bottom: headerRule.bottom }, spacing: { after: 240 }, run: { color: color(tokens.muted), size: 19 } });
  ...
  if (coverLetterRecipientAddress(plan.target)) paragraph(coverLetterRecipientAddress(plan.target), { spacing: { after: 30 } });
  ...
  plan.paragraphs.forEach((entry) => paragraph(entry.text, { keepLines: true, spacing: { before: 0, after: Math.round(tokens.coverLetterParagraphAfterPt * 20) } }));
  paragraph(plan.signoff, { keepNext: true, spacing: { before: 80, after: 35 } });

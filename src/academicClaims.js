...
export function academicStatusIssues(proposed, sources = []) {
  ...
  // Work verbs in a different sentence do not describe qualification status.
  // Keep abbreviated degrees together when locating sentence boundaries.
  const protectedText = String(proposed || '').replace(/\b(?:B\.?Sc\.?|M\.?Sc\.?|B\.?A\.?|M\.?A\.?|Ph\.?D\.?)\b\.?/g, value => value.replaceAll('.', ''));
  const issues = [];
  for (const text of protectedText.split(/[.!?;\n]+/).filter(clause => qualification.test(clause))) {
    const words = terms(text);
    const relevant = lines.filter(line => terms(line).some(word => words.includes(word)));
    if (ongoing.test(text) && !relevant.some(line => ongoing.test(line))) issues.push('Preserve the qualification status: its date alone does not establish that the candidate is completing or pursuing it.');
    if (/\b(?:graduate|graduated|earned|completed|hold|awarded)\b/i.test(text) && relevant.length
      && relevant.every(line => ongoing.test(line))) issues.push('An in-progress qualification cannot be presented as completed or awarded.');
  }

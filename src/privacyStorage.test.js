...
test("local deletion is allowlisted to the current account's private resume records", () => {
  const storage = storageFrom([
    ...
    ["gigscapes:application-package:v1:user-1:listing%3A1", "{}"],
  ...
  assert.equal(privateBrowserDataKeys("user-1", storage).length, 8);
  assert.deepEqual(clearPrivateBrowserData("user-1", storage), { ok: true, removed: 8 });

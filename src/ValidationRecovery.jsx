...
export default function ValidationRecovery() {
  ...
  const [status, setStatus] = useState('Ready to run 4 frozen first requests.');
  ...
  async function run() {
    ...
    setRunning(true);
    const captured = [];
    for (const cell of recoveryQueue.filter(c=>['R06','R08'].includes(c.caseId))) {
      if (stop.current) break;
      setStatus(`${captured.length}/4 complete. Running ${cell.caseId} ${cell.arm} ${cell.kind}.`);
      try {
        ...
        // Access/transport errors stop the queue; do not burn 4 failures on a broken runner.
        stop.current = true;
      }
      setResults([...captured]);
    }
    setRunning(false);
    setStatus(`${captured.length}/4 complete.${stop.current ? ' Stopped; captured attempts are preserved.' : ' Experiment complete.'}`);
  }
  function download() {
  ...
  return <main style={{ maxWidth: 960, margin: '40px auto', padding: 24, fontFamily: 'system-ui' }}>
    ...
    <button disabled={running || !results.length} onClick={async()=>{setStatus("Building document downloads…");const report=await recoveryDownload(results);setStatus("Exports: "+report.filter(r=>r.exported).length+"/4 documents. See downloaded export report.");}}>Download DOCX and PDF bundle</button>

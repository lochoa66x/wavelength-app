...
export function AtsReview({ review, C }) {
  ...
  const riskView = buildApplicationRiskView(review);
  ...
  const materialSafetyFallback = Boolean(safetyFallback?.applied && (
  ...
  return (
    <section aria-label="Tailoring quality review" style={{ background: panelBackground, border: `1px solid ${panelBorder}`, borderRadius: 14, padding: "14px 16px", marginBottom: 14 }}>
      ...
      <StatusRow label="Core requirement coverage" value={coverage.total ? `${coverage.total} core requirements assessed` : postingComplete ? "Requirement analysis incomplete" : "Limited by posting data"} detail={`${coverage.direct || 0} direct · ${coverage.adjacent || 0} adjacent · ${coverage.transferable || 0} transferable · ${coverage.missing || 0} unsupported`} ok={Boolean(coverage.total) && (coverage.missing || 0) === 0} C={C} />

  ...
  return (
    <details style={{ background: C.bgCard, border: `1px solid ${C.border}`, borderRadius: 14, margin: "0 0 12px", padding: "11px 13px" }}>
      ...
      <p style={{ color: C.textSub, fontSize: 11.75, lineHeight: 1.5, margin: "7px 0 0" }}>
        Review the sources and responsibility level behind each change. Your saved base résumé is unchanged. Choosing the verified original makes this export preliminary until the tailored wording is restored or the document is rechecked.
      </p>
          ...
          return (
            ...
            <article style={{ border: `1px solid ${C.border}`, borderRadius: 10, padding: "10px 11px" }}>
              ...
              <div style={{ display: "grid", gap: 7, marginTop: 8 }}>
                <div style={{ background: C.bg, borderRadius: 8, color: C.textSub, fontSize: 11.5, lineHeight: 1.45, padding: "7px 8px" }}><strong style={{ color: C.text }}>Source wording:</strong> {change.original}</div>
              ...
              {citations.length ? (
                <p style={{ color: C.textFaint, fontSize: 10.75, lineHeight: 1.45, margin: "5px 0 0" }}>
                  Source context: {change.source_role || "Check the source engagement"}. {citations.map((citation) => `“${citation.excerpt}”${citation.line_index ? ` (line ${citation.line_index})` : ""}`).join(" · ")}
                </p>
